  ...
  */
 int brightness[3] = {100,150,200,250};
 ...
 */
 ...
 // TIMER2: Interrupt Service Routine called on Compare Match
 ISR(TIMER2_COMPA_vect ) {
     // Disable interrupt when getting into the Interrupt Handler
     cli();
     timer3_count++;
     ...
     if(timer3_count == 1000){
        ...
        ADMUX |= (1 << REFS0) | 0x1;
        // Set registers to start single conversion
        ADCSRA |= (1 << ADEN) | (1 << ADPS2) | (ADPS1) | (ADPS0);
        ADCSRA |= (1 << ADSC);

        // Intialize 25bit buffer to hold value of IR Sensor
        char buffer[25];
        memset(buffer, ' ', 25);
     ...
     // Re-Enable interrupts when we're done with the Interrupt Handler
     sei();
 }
 ...
 */
 ...
 /*
    This does the setup for Timer2:
        - Fast PWM
        - Toggle OC2A on Compare Match
 */
 void setupTimerTwo(){
     // Fast PWM and toggle OC2A on compare match 
